...
export async function POST(req: Request) {
  const { id, item_name, description, category, price, stock_quantity, reorder_level, last_restock_date: last_restock_date_str,
    Image_url, barcode } = await req.json();
  ...
  try {
    const response = await editItem(item_name, description, category, price, stock_quantity, reorder_level, last_restock_date, Image_url, id, barcode);
