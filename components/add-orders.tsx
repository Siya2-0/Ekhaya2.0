...
const NewOrderManagement = ({ categoriesData, itemsData, username, setIsAddMoreOpen }: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center overflow-auto z-50">
        <div className="max-h-[80%] overflow-auto bg-[#F2F2F2] z-60">
        <Toaster />
        {showOrders && (
            <div
        ...
        <header className="bg-[#F2F2F2] shadow-md p-4 sticky top-0 z-50">
            ...
            <div className="flex">
                ...
                <button onClick={() => setIsAddMoreOpen(false)} className="text-gray-500 hover:text-gray-700 w-fit ml-8">
                    <FaTimes size={32} />
                </button>
            </div>
            </div>
        ...
        <div className={`fixed top-0 right-0 h-full w-full md:w-[700px] bg-[#f2f2f2] z-50 shadow-lg transform transition-transform duration-300 ${showOrders ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <React.Fragment>
                    ...
                    <div className='justify-center items-center text-center absolute bottom-0 w-full'>
                        ...
                        <span className={`absolute bottom-0 left-0 right-0 top-0 z-10 flex h-full w-full items-center justify-center ${(currentOrders.length === 0) ? "group-hover:text-[#898989]":"group-hover:text-[#ffffff]"} text-[18px] font-semibold`}>Continue Adding</span>
                        </button>
                        ...
                        <span className={`absolute bottom-0 left-0 right-0 top-0 z-10 flex h-full w-full items-center justify-center ${(currentOrders.length === 0) ? "group-hover:text-[#ffffff]":"group-hover:text-[#D62929]"} text-[18px] font-semibold`}>Finish</span>
                        </button>
                    </div>
                    </div>
                </React.Fragment>
